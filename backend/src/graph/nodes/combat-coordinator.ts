 ...
 */
export function combatCoordinatorNode(state: GameState): Partial<GameState> {
  ...
  if (combatState && combatState.isCombatOver) {
    ...
    const updatedCreatures = creatures.map(creature => {
      const combatChar = combatState.characters.find(
        c => !c.isPlayer && c.name === creature.name
      );
 ...
 */
export function shouldStartCombat(state: GameState): boolean {
