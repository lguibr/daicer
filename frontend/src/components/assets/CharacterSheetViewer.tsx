...
export function CharacterSheetViewer({ characterData, onClose }: CharacterSheetViewerProps) {
    ...
    const prof = form.proficienciesAndLanguages?.split(',').map((p) => p.trim()).filter(Boolean) || [];
                        ...
                        <Card className="p-4 bg-midnight-800/50 border-accent/20">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                ...
                                <div className="text-center p-3 bg-midnight-900/50 rounded-lg">
                                    ...
                                    <div className="text-2xl font-bold text-white">
                                        {10 + getModifier(getAttr('dexterity'))}
                                    </div>
                                </div>
                                <div className="text-center p-3 bg-midnight-900/50 rounded-lg">
                                    <div className="text-xs text-shadow-400 mb-1">Initiative</div>
                                    <div className="text-2xl font-bold text-white">
                                        {getModifier(getAttr('dexterity'))}
                                    </div>
                                </div>
                                <div className="text-center p-3 bg-midnight-900/50 rounded-lg">
                            ...
                            <Card className="p-4 bg-midnight-800/50 border-accent/20">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    {skills.map(([skill, bonus]) => (
                                        <div
                                            key={skill}
                                            className="flex items-center gap-2 p-2 bg-midnight-900/50 rounded text-sm"
                                        >
                            ...
                            {form.appearance && (
                                <div>
                                    ...
                                    <div className="text-xs text-shadow-300 space-y-1">
                                        {form.appearance.age && <p><span className="text-accent">Age:</span> {form.appearance.age}</p>}
                                        {form.appearance.height && <p><span className="text-accent">Height:</span> {form.appearance.height}</p>}
                                        {form.appearance.weight && <p><span className="text-accent">Weight:</span> {form.appearance.weight}</p>}
                                        {form.appearance.eyes && <p><span className="text-accent">Eyes:</span> {form.appearance.eyes}</p>}
                                        {form.appearance.skin && <p><span className="text-accent">Skin:</span> {form.appearance.skin}</p>}
                                        {form.appearance.hair && <p><span className="text-accent">Hair:</span> {form.appearance.hair}</p>}
                                        {form.appearance.description && (
                                            <p className="mt-2">{form.appearance.description}</p>
                                        )}
                                    </div>
                                </div>
                            )}
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-shadow-300 space-y-2">
                                        {form.personality.traits && (
                                            <p><span className="text-accent">Traits:</span> {form.personality.traits}</p>
                                        )}
                                        {form.personality.ideals && (
                                            <p><span className="text-accent">Ideals:</span> {form.personality.ideals}</p>
                                        )}
                                        {form.personality.bonds && (
                                            <p><span className="text-accent">Bonds:</span> {form.personality.bonds}</p>
                                        )}
                                        {form.personality.flaws && (
                                            <p><span className="text-accent">Flaws:</span> {form.personality.flaws}</p>
                                        )}
                                    </div>
                                </div>
                            )}
