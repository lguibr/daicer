        ...
        (playerX: number, playerY: number) => {
            ...
            for (let dy = -chunkRadius; dy <= chunkRadius; dy++) {
                for (let dx = -chunkRadius; dx <= chunkRadius; dx++) {
                    ...
                    if (overlapsInitial) {
                        console.log(`[SimpleTerrainManager] Skipping chunk (${cx}, ${cy}) at (${chunkStartX}, ${chunkStartY}) - overlaps initial grid area`);
                        loadedChunksRef.current.add(key); // Mark as checked
                        continue;
                    }

                    console.log(`[SimpleTerrainManager] Loading chunk (${cx}, ${cy}) at world pos (${chunkStartX}, ${chunkStartY})`);
