...
export function useExplorationMap(options: UseExplorationMapOptions = {}) {
  const { visionRadius = 40, chunkSize = 16, initialPosition = { x: 64, y: 64, z: 0 } } = options;
  ...
  const fetchChunk = useCallback(
    async (chunkX: number, chunkY: number, z: number) => {
      ...
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/explore/chunk/${worldX}/${worldY}/${z}`);
