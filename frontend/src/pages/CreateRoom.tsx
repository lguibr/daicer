...
export function CreateRoomPage() {
  ...
  const OptionButton: React.FC<{ onClick: () => void; isActive: boolean; children: React.ReactNode }> = ({
    ...
    <button
      ...
      className={`flex-1 p-2 rounded-md text-sm transition-colors ${
        isActive ? 'bg-cyan-600 text-white font-bold' : 'bg-slate-700 hover:bg-slate-600'
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4">
      <div className="w-full max-w-2xl p-8 space-y-8 bg-slate-800 rounded-xl shadow-lg relative">
        <div className="absolute top-4 right-4">
          <LanguageSelector />
        </div>
        
        <div className="text-center">
          <h1 className="text-4xl font-bold text-cyan-400 mb-2">{t('worldSettings.title')}</h1>
          <p className="text-slate-300">{t('worldSettings.subtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <h3 className="text-lg text-cyan-500 font-semibold">{t('worldSettings.story')}</h3>

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">{t('worldSettings.theme')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">{t('worldSettings.setting')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">{t('worldSettings.tone')}</label>
                <input
                  type="text"
                  value={settings.tone}
            ...
            {/* Scope Settings */}
            <div className="space-y-4">
              <h3 className="text-lg text-cyan-500 font-semibold">{t('worldSettings.scope')}</h3>

              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">{t('worldSettings.playerCount')}</label>
                <input
                  type="number"
                  min="1"
                  max="8"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">{t('worldSettings.adventureLength')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">{t('worldSettings.difficulty')}</label>
                <div className="flex space-x-2">
          ...
          <div className="flex gap-4">
            <button
              ...
              disabled={loading}
              className="flex-1 px-6 py-3 bg-slate-600 text-white font-bold rounded-lg shadow-md hover:bg-slate-700 transition-colors disabled:opacity-50"
            >
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-6 py-3 bg-cyan-600 text-white font-bold rounded-lg shadow-md hover:bg-cyan-700 transition-colors disabled:bg-slate-500 disabled:cursor-not-allowed"
            >
