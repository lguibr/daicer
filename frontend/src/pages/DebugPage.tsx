...
import type { WorldConfig as OldWorldConfig, Coordinates, Tile, Chunk } from '../features/debug/utils/types';
...
import { EntitySpawner } from '../features/debug/components/EntitySpawner';
...
// interface Coordinates removed (using imported)
...
export default function DebugPage() {
  ...
  const { socket } = useSocket(activeRoomId || undefined, 'debug-user');
  ...
  const handleSpawn = (target: Coordinates) => {
    if (!activeRoomId || !placementMode) return;
  ...
  const handleTileSingleClick = (target: Coordinates) => {
    ...
    // Open Context Menu
    setContextMenu(null);
    // ... (rest of context menu logic if any, currently mostly stubs or handled by renderer props)
    // Actually the previous implementation for context menu was relying on rendering a div if coords were set?
    // Wait, the previous code had `setContextMenu` logic inside `handleTileSingleClick`?
    // Yes, but I commented it out/simplified it in previous edit?
    // Re-reading previous file content:
    // It creates context menu if I implement it.
    // Current logic in `DebugPage.tsx` lines 105-129 was just `setContextMenu(null);` and comments.
    // I should probably support Context Menu properly later, but for now spawn takes precedence.
  };
  ...
  }, [visibleTiles, activeEntityId, activeEntity]); // Depend on visibleTiles which changes with position
  ...
  const handleExecuteTurn = async () => {
    if (activeRoomId) {
      ...
      try {
        await executeEngineAction(activeRoomId, actions);
      ...
      setPendingSpawns([]); // Clear local ghosts
      setErrorMsg('Turn Executed (Local)');
    }
  };
            ...
            <div className="flex flex-col gap-2 mb-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="h-8 text-xs bg-midnight-800 border-midnight-700"
                />
                <Button size="sm" variant="outline" className="h-8 px-2" onClick={() => setActiveRoomId(roomIdInput)}>
                  Load
                </Button>
              </div>
              {activeRoomId && <div className="text-[10px] text-green-400">Connected to: {activeRoomId}</div>}
            </div>
