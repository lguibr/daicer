...
export function createDie(type: DieType, color: string): THREE.Group {
  ...
  if (type === 2) {
    ...
    faceData.forEach(({ normal, center }, index) => {
      const plane = createNumberPlane(numbers[index], planeSize);
    ...
    for (let i = 0; i < faceCount; i += 1) {
      ...
      const plane = createNumberPlane(numbers[i], 0.5);
    ...
    for (let i = 0; i < faceCount; i += 1) {
      ...
      const plane = createNumberPlane(numbers[i], type === 4 ? 0.6 : 0.5);
      plane.position.copy(center).add(normal.clone().multiplyScalar(0.01));
      plane.lookAt(center.clone().add(normal));
      dieGroup.add(plane);
    }
